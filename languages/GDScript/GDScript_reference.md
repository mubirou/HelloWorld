...
```
（注意）内部では [randi()](https://docs.godotengine.org/ja/stable/classes/class_randomnumbergenerator.html#class-randomnumbergenerator-method-randi) 関数を利用（同じシード値）しているため同じ値が返るため場合によっては[乱数](#乱数)を使う必要がある  
[[配列TOP](#配列)]
